...
# ============================================
# FUNCIÓN PARA LANZAR CONTENEDOR (PARALELA)
# ============================================
def launch_container(args):
    """Lanza un contenedor Docker - se ejecuta en paralelo"""
    idx, chunk_file, temp_dir, results_dir, dataset_path = args
    
    cmd = [
        "docker", "run", "--rm",
        "-v", f"{os.path.abspath(temp_dir)}:/data",
        "-v", f"{os.path.abspath(results_dir)}:/results",
        "-v", f"{os.path.abspath(dataset_path)}:/fulldata/curse.csv:ro",  # ✅ Path corregido
        "ml_node",
        "python", "/app/train_node.py", f"/data/{os.path.basename(chunk_file)}"
    ]
    ...
    if result.returncode != 0:
        print(f"❌ Nodo {idx} FALLÓ:")
        print(f"   STDOUT: {result.stdout[:200]}")
        print(f"   STDERR: {result.stderr[:200]}")
    else:
        print(f"✓ Nodo {idx} completado")
        if result.stdout:
            print(f"   Output: {result.stdout[:100]}")
    
    return idx, result.returncode, result.stdout, result.stderr
...
if __name__ == '__main__':
    ...
    for n_nodes in nodes_list:
        ...
        # ============================================
        # LANZAR CONTENEDORES EN PARALELO CON Pool
        # ============================================
        print(f"\n🚀 Lanzando {len(chunk_files)} contenedores Docker EN PARALELO...")
        start_time = time.time()
        
        # Preparar argumentos para cada contenedor
        args_list = [
            (i, chunk_file, temp_dir, results_dir, dataset)
            for i, chunk_file in enumerate(chunk_files)
        ]
        
        # Pool ejecuta todos los contenedores simultáneamente
        with Pool(processes=len(chunk_files)) as pool:
            pool_results = pool.map(launch_container, args_list)
        
        makespan_real = time.time() - start_time
        print(f"⏱️  Makespan real (wall-clock): {makespan_real:.2f}s")
        ...
        makespan = max(times)  # Makespan = tiempo del worker más lento
        print(f"\n📊 Makespan (max worker time): {makespan:.2f}s")
